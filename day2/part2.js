...
function passwordValidator() {
    let passwordsMapped = passwords.map(p => p.split(" "));

    for (i = 0; i < passwordsMapped.length; i++) {
        let numOfOccurrences = 0;
        
        // split passwordsMapped[i][0] by '-' to get the integers
        let integerStrings = passwordsMapped[i][0].split("-");

        // parse each string value into integer data type using parseInt()
        // subtract one for consistency with string indexing (count begins
        // at zero, not one).
        let firstPosition = parseInt(integerStrings[0]) - 1;
        let secondPosition = parseInt(integerStrings[1]) - 1;

        // passwordsMapped[i][1][0] for extracting character to be validated,
        // minus the colon
        let charToCheck = passwordsMapped[i][1][0];

        // passwordsMapped[i][2] is our password
        let passwordToValidate = passwordsMapped[i][2];

        // extract characters at specified positions in pwd:
        let firstCharacter = passwordToValidate[firstPosition];
        let secondCharacter = passwordToValidate[secondPosition];

        // compare charToCheck with firstCharacter
        // increase count if true
        if (firstCharacter === charToCheck) {
            numOfOccurrences++;
        }

        // compare charToCheck with secondCharacter
        // increase count if true
        if (secondCharacter === charToCheck) {
            numOfOccurrences++;
        }

        // if count=0, invalid. if count=1, valid. if count=2, invalid.
        // count valid passwords in global counter variable
...
// Advent of Code 2020 Day 2, Part 2: Password Philosophy
//                  _...Q._
//                .'       '.
//               /           \
//              ;.-""""--.._ |
...
//              \' '   _.-'
//               \ _.-'
// ASCII Snowman by Joan Stark at
